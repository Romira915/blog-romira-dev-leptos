...
以下はテストケースとして記載されているが、現在の実装では実行不可能です。

- TC-302 OAuth未設定時の動作 — OAuth環境変数は全て必須（未設定時は起動時パニック）
...
| TC-117 | F-103 | 異常系 | 公開記事保存: スラッグ空でバリデーションエラー |
| TC-118 | F-103 | 正常系 | エディタ: Markdownプレビューの表示確認 |
| TC-119 | F-103 | 正常系 | エディタ: フルスクリーンモードの切り替え |
| TC-120 | F-101 | 正常系 | 記事一覧: 新規作成ボタンでエディタに遷移 |
| TC-121 | F-109 | 正常系 | カバー画像設定: 画像ライブラリから選択してカバー画像を設定 |
| TC-122 | F-109 | 正常系 | カバー画像設定: 設定済みカバー画像の変更 |
| TC-123 | F-109 | 正常系 | カバー画像設定: カバー画像の解除 |
| TC-124 | F-110 | 正常系 | 本文画像挿入: 画像ライブラリから選択してHTML挿入 |
| TC-125 | F-110 | 正常系 | 本文画像挿入: 挿入されたHTMLがプレビューで画像表示される |
...
| TC-211 | F-106 | 境界値 | 画像アップロード: 10MBちょうどで成功 |
...
| TC-302 | F-201 | ~~正常系~~ | ~~OAuth: 未設定時は「認証なし」表示で操作可能~~ **テスト不可** |
...
| TC-308 | F-201 | 異常系 | ADMIN_EMAILS: 許可外メールアドレスで403エラー |
| TC-309 | F-201 | 正常系 | ADMIN_EMAILS: 許可メールアドレスでアクセス成功 |

### 管理機能 — 未実装 — [admin-unimplemented.md](./admin-unimplemented.md)

| TC-ID | 機能ID | カテゴリ | 概要 |
|-------|--------|---------|------|
| TC-501 | F-111 | 正常系 | 記事削除: 下書き記事の削除 |
| TC-502 | F-111 | 正常系 | 記事削除: 公開記事の削除 |
| TC-503 | F-111 | 正常系 | 記事削除: 削除後に記事一覧から消える |
| TC-504 | F-111 | 異常系 | 記事削除: 存在しない記事IDで削除を試みる |
