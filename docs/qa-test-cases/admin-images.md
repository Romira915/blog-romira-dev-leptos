...
- 管理画面にアクセス可能である
- GCSの署名付きURL生成が正常に動作する
- ちょうど10MB（10,485,760バイト）のJPEG画像ファイルを用意する

### 手順

1. `http://127.0.0.1:3000/admin/images` にアクセスする
2. 10,485,760バイトのJPEG画像をアップロードする
...
- 有効範囲: 1 ≤ `size_bytes` ≤ 10,485,760（10MB）
- 10,485,760バイトちょうどは有効、10,485,761バイト以上はエラー（TC-205）
- TC-205（>10MB エラー）と対になる境界値テスト
