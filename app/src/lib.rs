    ...
    view! {
        ...
        <Suspense fallback=|| ()>
            {move || Suspend::new(async move {
                let number = random_number().await.unwrap_or_default();
               view! {
                 <Meta name="og:title" content={number.to_string()} />
               }
            })}
        </Suspense>
        <Router>
            ...
            <main>
                <FlatRoutes fallback>
                    // We窶冤l load the home page with out-of-order streaming and <Suspense/>
...
fn HomePage() -> impl IntoView {
    ...
    view! {
        <h1 class=my_style::jumbotron>"My Great Blog"</h1>
