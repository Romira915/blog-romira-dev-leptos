...
impl GcsSigningService {
    ...
    pub async fn generate_upload_url(
        ...
        content_type: &str,
        duration_secs: u64,
    ) -> Result<String, GcsSigningError> {
        let signer = self
            .signer
            .as_ref()
            .ok_or_else(|| GcsSigningError::SigningError("Signer not configured".to_string()))?;
        ...
        .await
        .map_err(|e| GcsSigningError::SigningError(format!("Task join error: {}", e)))?
        .map_err(|e| GcsSigningError::SigningError(e.to_string()))
    }
